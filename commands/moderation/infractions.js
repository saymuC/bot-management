...
async function undoPunishments(guild, user, moderator, reason) {
  const lines = [];
  const mute = emoji(guild, 'mute');
  const ban = emoji(guild, 'ban');
  let changed = false;
  ...
  if (!member) {
    lines.push(`${mute} **Silenciamento:** o membro não está no servidor.`);
  } else if (!member.isCommunicationDisabled()) {
    lines.push(`${mute} **Silenciamento:** não havia nenhum ativo.`);
  } else if (!member.moderatable) {
    lines.push(`${mute} **Silenciamento:** ativo, mas não alcanço o membro (hierarquia de cargos).`);
  } else {
    try {
      await member.timeout(null, why);
      lines.push(`${mute} **Silenciamento:** removido.`);
      changed = true;
    } catch (err) {
      lines.push(`${mute} **Silenciamento:** falhou — ${err.message}`);
    }
  }

  // `bans.fetch` de um id não banido devolve erro; é assim que se consulta.
  const activeBan = await guild.bans.fetch(user.id).catch(() => null);

  if (!activeBan) {
    lines.push(`${ban} **Banimento:** não havia nenhum.`);
  } else if (!moderator.permissions.has(PermissionFlagsBits.BanMembers)) {
    lines.push(`${ban} **Banimento:** ativo, mas desbanir exige a permissão de **Banir membros**.`);
  } else {
    try {
      await guild.bans.remove(user.id, why);
      lines.push(`${ban} **Banimento:** removido.`);
      changed = true;
    } catch (err) {
      lines.push(`${ban} **Banimento:** falhou — ${err.message}`);
    }
  }
...
async function notifyPardon(guild, user, reason, done) {
  try {
    await user.send({
      embeds: [
        baseEmbed({
          title: `${emoji(guild, 'pardon')} Suas infrações foram perdoadas`,
          description:
    ...
    return `${emoji(guild, 'dm_notice')} **Aviso na DM:** enviado.`;
  } catch {
    return (
      `${emoji(guild, 'dm_notice')} **Aviso na DM:** não foi possível enviar ` +
      '(DM fechada, ou o bot não divide servidor com o usuário).'
    );
  }
}

/** Uma linha do histórico. */
function describeRow(row, expireMs, now, expiredMark) {
  ...
  return `${expired ? expiredMark : '•'} **${label}** (${row.points} pt${row.points === 1 ? '' : 's'}${expired ? ', vencido' : ''})${action}\n` +
  ...
  async execute(interaction) {
    ...
    if (shouldClear || shouldUndo) {
        ...
        return respond(interaction, {
          embeds: [
            errorEmbed(
              ...
              `${emoji(interaction.guild, 'warning')} Motivo obrigatório`
            ),
          ],
        });
      }
      ...
      if (shouldClear) {
        ...
        parts.push(
          `${emoji(interaction.guild, 'clear')} **Pontos:** ${removed} infração(ões) apagadas, pontuação de volta a zero.`
        );
      }
      ...
      return respond(interaction, {
        embeds: [
          successEmbed(
            `**Motivo:** ${reason}\n\n${[...parts, notice].filter(Boolean).join('\n')}`,
            `${emoji(interaction.guild, 'pardon')} Perdão aplicado a ${user.tag}`
          ),
        ],
      });
    }
    ...
    const expiredMark = emoji(interaction.guild, 'timer');

    if (rows.length) {
      fields.push({
        name: `Últimas ${rows.length} de ${total}`,
        value: rows
          .map((row) => describeRow(row, expireMs, now, expiredMark))
          .join('\n')
          .slice(0, 1024),
        inline: false,
      });
    ...
    return respond(interaction, {
      embeds: [
        baseEmbed({
          ...
          description: total
            ? `As linhas com ${expiredMark} já venceram e não contam para a escada, mas ficam no histórico.`
